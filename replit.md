...
**Security (November 2025)**: Comprehensive security hardening implemented with:
-   **Authentication & Authorization**: All data modification endpoints (POST/PATCH/DELETE) require user login via Passport.js with bcrypt password hashing
-   **CSRF Protection**: Double-submit cookie pattern protects against Cross-Site Request Forgery attacks with automatic token management
-   **Rate Limiting**: Four-tier system - Aggressive (login/register: 5/15min), Strict (uploads: 10/hour), Moderate (mutations: 100/15min), Light (reads: 1000/15min)
-   **Security Headers**: Helmet.js provides CSP, HSTS, X-Frame-Options, X-Content-Type-Options, XSS filters
-   **Audit Logging**: All sensitive operations logged with user ID, timestamp, IP, action, and success/failure status (last 10,000 entries in memory)
-   **Session Security**: Configurable SESSION_SECRET (must be set in production), secure cookies with httpOnly and sameSite flags
-   Public endpoints: GET requests and POST /api/page-views remain public for viewing data and analytics tracking
...
**Parliamentary Activity Extraction** (November 2025): Comprehensive system to extract and track Bills, Motions, and Questions from Hansard PDFs. Three specialized parsers work together: (1) `HansardSectionParser` identifies PERTANYAAN (questions), RANG UNDANG-UNDANG (bills), and USUL (motions) sections using regex patterns, (2) `HansardQuestionParser` extracts oral/written questions with MP attribution, ministry targets, and question text, (3) `HansardBillMotionParser` extracts bills with bill numbers, sponsors, co-sponsors, and motions with proposers. All extracted data is automatically saved to database during PDF upload with MP linkage via constituency-based matching. API endpoints available at `/api/parliamentary-questions` and `/api/legislative-proposals` for querying by MP or retrieving all records.
...
**Legislative Proposal Schema**: `id`, `mpId`, `title`, `description`, `type` (bill/motion), `status`, `proposedDate`, `billNumber`, `coSponsors`, `hansardRecordId`.
**Parliamentary Question Schema**: `id`, `mpId`, `questionText`, `ministry`, `questionDate`, `questionType` (oral/written), `questionNumber`, `hansardRecordId`.
