 ...
 * Uses only built-in Node.js fetch API
 */
...
async function fetchOfficialContactInfo(parliamentCode) {
  try {
    ...
    // Extract Address using simple table parsing
    const tableRows = html.match(/<tr[^>]*>[\s\S]*?<\/tr>/g) || [];
    for (const row of tableRows) {
      const cells = row.match(/<td[^>]*>([\s\S]*?)<\/td>/g) || [];
      if (cells.length >= 2) {
        const labelCell = cells[0].replace(/<[^>]*>/g, '').trim().toLowerCase();
        const valueCell = cells[1].replace(/<[^>]*>/g, '').trim();

        if (labelCell.includes('alamat surat') && !contactInfo.address) {
          contactInfo.address = valueCell;
        } else if (labelCell.includes('alamat') && !contactInfo.address) {
          contactInfo.address = valueCell;
        }

        if ((labelCell.includes('kementerian') || labelCell.includes('menteri')) && !contactInfo.ministryOffice) {
